...
def generate_episode_descriptions(task_name: str, episodes: List[Dict[str, str]], max_descriptions: int = 1000000):
    ...
    for i, episode in enumerate(episodes):
        # episode = {k: v for k, v in episode.items() if k.startswith('{') and k.endswith('}')}
        episode = {
            k: v
            for k, v in episode.items()
            if k.startswith("{")
            and k.endswith("}")
            and len(k[1:-1].strip()) == 1
            and k[1:-1].strip().isalpha()
        }
