...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # Default stage label used for per-frame metadata export.
        self.current_stage = "default"

        self.need_topp = True  # TODO

        # Audio configuration.
        self.audio_config = kwags.get("audio", {})
        self.collect_audio = self.audio_config.get("collect_audio", False) or self.data_type.get("audio", False)
        self.enable_audio_speaker = bool(self.audio_config.get("enable_speaker", False))
        if self.enable_audio_speaker and not self.render_freq:
            print("Warning: enable_speaker is ignored because render_freq is 0.")
            self.enable_audio_speaker = False
        
        # Flag used by motion routines that should stop after audio playback.
        self.audio_finished_interrupt = False

        # Video export configuration.
        video_config = kwags.get("video", {})
        self.video_render_mode = video_config.get("render_mode", "full")
        self.save_video_output = video_config.get("save_video", True)
        ...
        # sim_timestep=self.scene.get_timestep(),
        
        # Initialize audio after the scene, robot, and cameras are ready.
        if self.collect_audio:
            self.load_audio(**kwags)
        ...
        # Reset the audio buffer after scene initialization so exported episode
        # audio starts from the actual task rollout rather than setup motions.
    ...
    def load_audio(self, **kwags):
        audio_config = kwags.get("audio", {})
        speaker_enabled = getattr(self, "enable_audio_speaker", False)
        # sim_timestep=self.scene.get_timestep(),
        self.audio = Audio(
            audio_length=audio_config.get("audio_length", 60.0),
            audio_sample_rate=audio_config.get("audio_sample_rate", 16000),
            sim_timestep=self.scene.get_timestep(),
            enable_speaker=speaker_enabled,
            visualizer_window_seconds=audio_config.get("visualizer_window_seconds", 2.0),
            loop_audio=audio_config.get("loop_audio", True),
            **kwags
        )

    def _handle_step_audio(self):
        """Task-specific per-step audio hook."""
        return
    ...
    def get_obs(self):
        ...
        # Store the recent audio window for each saved frame.
        if self.data_type.get("audio", False) and self.collect_audio:
            audio_config = getattr(self, 'audio_config', {})
            collect_audio_length = audio_config.get('collect_audio_length', 0.5)

            pkl_dic["audio"] = self.audio.get_recent_audio(collect_audio_length)

            audio_status = self.audio.get_status()
            audio_status['sample_rate'] = self.audio.sample_rate
            pkl_dic["audio_status"] = audio_status
    ...
    def _take_picture(self):  # save data
        ...
        # Persist the stage label with each frame.
        pkl_dic["stage"] = str(getattr(self, "current_stage", "default"))
    ...
    def merge_pkl_to_hdf5_video(self):
        ...
        # Pass render mode through to the video export helper.
        render_mode = getattr(self, 'video_render_mode', 'full')
        
        episode_audio = None
        episode_audio_sr = None
        if self.collect_audio and hasattr(self, "audio"):
            episode_audio = self.audio.export_episode_audio()
            episode_audio_sr = self.audio.sample_rate
        os.makedirs(f"{self.save_dir}/data", exist_ok=True)
        if not self.save_video_output:
            print("Video export disabled (video.save_video=false); skipping mp4 generation.")
        process_folder_to_hdf5_video(
            cache_path,
            target_file_path,
            target_video_path,
            render_mode=render_mode,
            full_episode_audio=episode_audio,
            audio_sample_rate=episode_audio_sr,
            save_video=self.save_video_output,
        )
    ...
    # Update the stage label used by subsequent saved frames.
    def set_stage_label(self, stage: str | None = None):
        self.current_stage = "default" if not stage else str(stage)
    ...
    def delay(self, delay_time, save_freq=None):
        ...
        for i in range(delay_time):
            ...
            # Keep audio capture/playback in sync during blocking waits.
            if self.collect_audio:
                self.audio.update()
    ...
    def together_move_to_pose(
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            # Keep audio capture/playback in sync during coordinated pose moves.
            if self.collect_audio:
                self.audio.update()
    ...
    def move(
        ...
        save_freq=-1,
        interrupt_on_audio_finish=False,
    ):
        """
        ...
        for left, right in zip(left_actions, right_actions):
            ...
            self.take_dense_action(control_seq, interrupt_on_audio_finish=interrupt_on_audio_finish)
            
            # Check for interruption after taking dense action
    ...
    def take_dense_action(self, control_seq, save_freq=-1, interrupt_on_audio_finish=False):
        """
        control_seq:
        ...
        for control_idx in range(max_control_len):
            ...
            # Check for audio interruption
            if interrupt_on_audio_finish and self.collect_audio and hasattr(self, "audio") and self.audio is not None:
                if self.audio.is_playing:
                    self.audio_finished_interrupt = True
            ...
            # Audio state must advance at the simulation step rate.
            if self.collect_audio:
                self.audio.update()
        ...
        if save_freq != None and not interrupted_by_audio:
            self._take_picture()
    ...
    def take_action_direct(self, action):
        """
        Execute one action without running motion planning.

        Args:
            action (np.ndarray): Action vector in the form
        ...
        eval_video_freq = 1  # fixed
        if (self.eval_video_path is not None and self.take_action_cnt % eval_video_freq == 0):
            if "observation" in self.now_obs and "head_camera" in self.now_obs["observation"]:
                self.eval_video_ffmpeg.stdin.write(self.now_obs["observation"]["head_camera"]["rgb"].tobytes())
        ...
        target_left_qpos = action[:left_arm_dim]
        target_left_gripper = action[left_arm_dim]
        target_right_qpos = action[left_arm_dim + 1 : left_arm_dim + right_arm_dim + 1]
        target_right_gripper = action[left_arm_dim + right_arm_dim + 1]

        self.robot.set_arm_joints(target_left_qpos, np.zeros_like(target_left_qpos), "left")
        self.robot.set_gripper(target_left_gripper, "left")

        self.robot.set_arm_joints(target_right_qpos, np.zeros_like(target_right_qpos), "right")
        self.robot.set_gripper(target_right_gripper, "right")
