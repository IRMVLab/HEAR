...
class open_microwave_audio(Base_Task):

    def setup_demo(self, is_test=False, **kwags):
        super()._init_task_env_(**kwags)
        self.microwave_audio_path = os.path.join("assets", "audios", "ding.mp3")
    ...
    def play_audio(self, loop_audio=False, randomize_start=False, delay=0.0):
        if not getattr(self, "collect_audio", False) or getattr(self, "audio", None) is None:
            return False
        if not os.path.exists(self.microwave_audio_path):
            print(f"Warning: missing audio file {self.microwave_audio_path}")
            return False
        success = self.audio.start_playing(
            self.microwave_audio_path,
    ...
    def play_once(self):
        ...
        self.set_stage_label("audio")
        audio_started = self.play_audio(loop_audio=False, randomize_start=False)
        reaction_time = float(np.random.uniform(1.0, 2.0))
        self._wait_and_update(reaction_time, save_freq=self.save_freq)

        self.set_stage_label("open")
        ...
        if not self.check_success(target=0.1):
    ...
    def check_success(self, target=0.1):
        ...
        success = qpos[0] >= limits[0][1] * target
        if success:
            self.stage_success_tag = True
            self.set_stage_label("done")
        if (
            success
            and getattr(self, "collect_audio", False)
