...
class touch_plate_metal(Base_Task):
    """
    ...
    def load_actors(self):
        ...
        self.plate_assignment = self._init_plate_assignment({"metal": "a", "plastic": "b"})
    ...
    def play_once(self):
        ...
        placed_on_metal = target_a["surface"] == "metal"

        if not placed_on_metal:
    ...
    def check_success(self):
        metal_pos = self.plate_assignment.get("metal") if hasattr(self, "plate_assignment") else None
        if metal_pos is None or not hasattr(self, "plates_by_position"):
            return False
        metal_actor = self.plates_by_position.get(metal_pos, {}).get("actor")
        if metal_actor is None:
            return False

        box_pos = self.box.get_functional_point(0, "pose").p
        target_pos = metal_actor.get_functional_point(1, "pose").p
        ...
        # If metal is placed at position b, placing the box on the metal plate is sufficient.
        if metal_pos == "b":
            return bool(box_on_metal)
