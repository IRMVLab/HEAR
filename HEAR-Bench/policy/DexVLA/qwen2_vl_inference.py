...
def load_model(model_path=None):
    if model_path is None:
        model_path = os.environ.get("QWEN2_VL_MODEL_PATH", "./Qwen2-VL-7B-Instruct")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Label clean-table episodes with Qwen2-VL.")
    parser.add_argument("--model-path", default=None, help="Path to the Qwen2-VL model directory.")
    parser.add_argument("--data-dir", required=True, help="Dataset root directory.")
    parser.add_argument("--tasks", nargs="+", required=True, help="One or more task directory names.")
    args = parser.parse_args()

    model, processor = load_model(args.model_path)
    for task in args.tasks:
        label_clean_tables(DATA_DIR=args.data_dir, task=task, model=model, processor=processor)
